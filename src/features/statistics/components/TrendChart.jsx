...
//    const currentTrend = trends?.trends ? trendData[selectedParameter] : {};
...
export const TrendChart = ({ trends, loading, className, ...props }) => {
  ...
  console.log("trend data from props", trends);
  
  // Update trendData whenever trends prop changes
  useEffect(() => {
    // Check the actual structure from your API
    if (trends?.data?.trends) {
      console.log("Updating trendData from data.trends:", trends.data.trends);
      setTrendData(trends.data.trends);
    } 
    // Alternative structure if trends.trends exists
    else if (trends?.trends) {
      console.log("Updating trendData from trends.trends:", trends.trends);
      setTrendData(trends.trends);
    }
    // If trends itself contains the trend data directly
    else if (trends && (trends.pH || trends.wqi || trends.tds || trends.turbidity)) {
      console.log("Updating trendData from trends directly:", trends);
      setTrendData(trends);
    }
  }, [trends]); // Re-run when trends object changes
  ...
  console.log("Current trend value:", currentTrend);
    ...
    <Card className={cn("bg-[#0a2540] border border-[#1e3a5f] shadow-lg", className)} {...props}>
      ...
      <CardContent>
        {currentTrend && Object.keys(currentTrend).length > 0 ? (
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="text-center p-4 bg-[#112b4a] rounded-lg">
                <p className="text-blue-300 text-sm mb-2">Start Value</p>
                <p className="text-2xl font-bold text-white">
                  {currentTrend.start !== undefined ? currentTrend.start.toFixed(2) : '—'}
                  {currentParam?.unit && <span className="text-sm text-blue-400 ml-1">{currentParam.unit}</span>}
                </p>
              </div>
              <div className="text-center p-4 bg-[#112b4a] rounded-lg">
                <p className="text-blue-300 text-sm mb-2">Current Value</p>
                <p className="text-2xl font-bold text-white">
                  {currentTrend.end !== undefined ? currentTrend.end.toFixed(2) : '—'}
                  {currentParam?.unit && <span className="text-sm text-blue-400 ml-1">{currentParam.unit}</span>}
                </p>
              </div>
              <div className="text-center p-4 bg-[#112b4a] rounded-lg">
                ...
                <div className="flex items-center justify-center gap-2">
                  {getTrendIcon(currentTrend.direction)}
                  <p className={cn("text-2xl font-bold", getTrendColor(currentTrend.direction, selectedParameter))}>
                    {currentTrend.change !== undefined ? (currentTrend.change > 0 ? '+' : '') + currentTrend.change.toFixed(2) : '0'}
                    {currentParam?.unit && <span className="text-sm ml-1">{currentParam.unit}</span>}
                  </p>
                </div>
                <p className="text-blue-400 text-xs mt-1 capitalize">
          ...
          <div className="text-center py-8">
            <p className="text-blue-400">No trend data available</p>
            <p className="text-blue-400 text-xs mt-2">Debug: {JSON.stringify(trendData)}</p>
          </div>
        )}
      </CardContent>
    </Card>
