...
export const RecentReadingsTable = ({ readings }) => {
  const getWQIStatus = (wqi) => {
    if (wqi >= 90) return { label: 'Excellent', color: 'text-green-600 bg-green-50' };
    if (wqi >= 70) return { label: 'Good', color: 'text-blue-600 bg-blue-50' };
    if (wqi >= 50) return { label: 'Fair', color: 'text-yellow-600 bg-yellow-50' };
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              {readings.map((reading, idx) => {
                const status = getWQIStatus(reading.wqi);
                return (
                  <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-slate-300">{reading.conductivity.toFixed(0)}</td>
                    <td className="px-6 py-4 font-semibold text-slate-300">{reading.wqi.toFixed(1)}</td>
